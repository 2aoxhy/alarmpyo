...
export const WORK_SETTINGS_SHARE_FORMAT_VERSION = 6 as const;
const LEGACY_WORK_SETTINGS_SHARE_FORMAT_VERSIONS = [1, 2, 3, 4, 5] as const;
...
function parsePattern(
  ...
  const kind = getWorkPatternKind(shiftTypeIds);
  if (!kind || item.name !== getWorkPatternName(kind)) {
...
function validateDocument(value: unknown): WorkSettingsSharePreview {
  ...
  if (
    root.formatVersion !== WORK_SETTINGS_SHARE_FORMAT_VERSION &&
    (!legacyFormat ||
      !LEGACY_WORK_SETTINGS_SHARE_FORMAT_VERSIONS.includes(
  ...
  if (!Array.isArray(workSettings.shiftTypes) || workSettings.shiftTypes.length !== SHARED_SHIFT_IDS.length) {
    throw new WorkSettingsShareValidationError('공유할 근무 종류가 모두 들어 있지 않아요.');
  }
  const shiftTypes = workSettings.shiftTypes.map(parseShiftSettings);
  const byId = new Map(shiftTypes.map((shift) => [shift.id, shift]));
  if (byId.size !== SHARED_SHIFT_IDS.length || SHARED_SHIFT_IDS.some((id) => !byId.has(id))) {
...
export function exportWorkSettingsToJson(data: AppData): string {
  ...
  // 앱 내부 데이터가 손상된 경우에도 잘못된 공유 파일을 만들지 않아요.
...
export function applyWorkSettingsPreview(
  ...
  const validated = validateDocument(preview.document).document.workSettings;
