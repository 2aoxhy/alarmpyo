...
- 휴대전화 스크린샷: `assets/play-store/phone-screenshots/manifest.json`에 기록한 V08 순서·대체 텍스트를 기준으로, 현재 릴리스 후보를 **Play 테스트 트랙으로 설치한 실제 Android 기기**에서 새로 촬영해요. manifest가 `recapture-required`인 동안은 최종 이미지가 없는 상태예요.
...
1. 실제 V08 시작 화면과 알람표 브랜드
2. 현재 근무·남은 시간·준비된 알람을 보여주는 오늘 화면
3. 실행 중인 30분 타이머
4. 근무표·알람·위젯·데이터로 이동하는 설정 화면

실기기에서 `01-brand.webp`, `02-today.webp`, `03-timer.webp`, `04-settings.webp`를 폭 1080px·높이 1920px 이상으로 다시 촬영한 뒤 다음 명령으로 무왜곡 RGB PNG를 만들어요. 더 긴 화면은 `#101214` 배경의 1080×1920 캔버스에 비율을 유지해 배치하며, 첨부 경로가 사라진 파일이나 이전 버전 이미지를 대신 사용하지 않아요.

```powershell
npm run assets:play:screenshots:prepare -- --source-dir "<V08 WebP 원본 폴더>"
```

Play Console에는 `manifest.json`의 순서와 한국어 대체 텍스트를 그대로 사용해요. 변환이 끝나기 전 manifest의 `recapture-required` 상태와 빈 스크린샷 폴더는 의도된 출시 차단 상태예요.

스크린샷은 현재 기능만 보여주고, 상태 표시줄의 개인정보·테스터 이메일·기기 식별 정보는 제거해요. 실제 앱의 다크 테마를 사용하고 화면을 합성해 존재하지 않는 기능을 만들지 않아요. 알림이나 통신사 이름이 보이지 않게 하고 배터리·Wi-Fi·셀룰러 상태 아이콘은 정상 상태로 촬영해요. 첫 장은 확정한 실제 시작 화면을 사용하고, 나머지 세 장은 서로 겹치지 않는 핵심 기능 화면을 사용해요.
...
- 현재 V08 후보 `1.0.8(8)`이 Play Console의 V07 `versionCode 7`보다 높은 같은 package 계보인지 확인
