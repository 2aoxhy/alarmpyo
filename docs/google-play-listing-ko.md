...
- 근무 패턴 파일 공유·적용 이력 복구와 전체 데이터 백업·복구
...
- 휴대전화 스크린샷: `assets/play-store/phone-screenshots/manifest.json`에 기록한 V12 순서·대체 텍스트를 기준으로, 현재 릴리스 후보를 **Play 테스트 트랙으로 설치한 실제 Android 기기**에서 새로 촬영합니다. manifest가 `recapture-required`인 동안은 최종 이미지가 없는 상태입니다.
...
```powershell
npm run assets:play:screenshots:prepare -- --source-dir "<V12 WebP 원본 폴더>"
```
...
- 현재 V12 후보 `1.0.12(12)`가 Play Console Alpha의 V08 `versionCode 8`보다 높은 같은 package 계보인지 확인합니다. V09는 사용하지 않습니다.
