...
1. 실제 실행 중인 Node.js가 `24.16.0`, npm이 `11.13.0`인지 확인해요.
2. Git 상태가 깨끗한지 확인해요.
3. TypeScript, lint, JavaScript 테스트를 실행해요.
4. 운영 의존성과 개발 도구 취약점을 감사해요.
5. Expo Doctor와 Android 네이티브 단위 테스트를 실행해요.
6. 패키지·버전·runtimeVersion·EAS project ID·canary 프로필을 검증해요.
7. `eas project:info`로 로그인 상태와 실제 원격 프로젝트 연결을 확인해요.
...
production Hosting URL, 공개 APK manifest, `public/downloads`, 실기기 배포 증거는 내부 APK 생성의 선결 조건이 아니에요. direct 인증서 신뢰 지문은 내부 canary에서 확정해 `release-policy.json`에 기록했지만, Hosting URL이 없으면 stable APK 승격과 공개는 계속 차단돼요.
...
- 내부 검증 결과만으로 `release-policy.json`을 활성화하지 않아요. production Hosting URL을 실제 값으로 확정해야 해요. Play App Signing은 direct 공개 정책과 별도인 Google 관리 signer로 결정됐으며, 내부 canary 결과로 그 값을 바꾸지 않아요.
